...
intro_message() {
    echo "========================================="
    echo "       🚀 Forge Minecraft Setup 🚀       "
    echo "========================================="
    sleep 1
    echo " This script automates the installation of a Forge Modded Minecraft server."
    echo "========================================="
    sleep 1
}
...
ask_naming_details() {
    ...
    echo "🔍 Scanning for existing '${MC_TYPE} ${FORGE_MC_VERSION} (${GAMEMODE})' servers..."
    existing_services=($(ls /etc/systemd/system/[0-9][0-9]-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}.service 2>/dev/null))
    ...
    if [ "$SELECTED_ACTION" == "DELETE" ]; then
        read -p "⚠️  ENTER FULL SERVICE NAME to DELETE (e.g., 01-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}): " target_name
        if [[ ! -f "/etc/systemd/system/${target_name}.service" ]]; then
             echo "❌ Service '$target_name' not found."
             exit 1
        fi
        
        if [[ "$target_name" != *"-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}" ]]; then
    ...
    if [ "$SELECTED_ACTION" == "REINSTALL" ]; then
        read -p "⚠️  ENTER FULL SERVICE NAME to REINSTALL (e.g., 01-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}): " target_name
        if [[ ! -f "/etc/systemd/system/${target_name}.service" ]]; then
             echo "❌ Service '$target_name' not found."
             exit 1
        fi
    ...
    if [ "$SELECTED_ACTION" == "CREATE" ]; then
        for i in {1..99}; do
            id_str=$(printf "%02d" $i)
            candidate_name="${id_str}-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}"
    ...
    MC_SERVICE_NAME="${SERVER_ID}-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}"
    FORGE_MC_DIR="${MC_DIR}/${SERVER_ID}-${MC_TYPE}-${FORGE_MC_VERSION}-${GAMEMODE}-${TIMESTAMP}"
    
    echo "✅ Configuration Set:"
    echo "   ID: $SERVER_ID"
    echo "   Service: $MC_SERVICE_NAME"
    echo "   Dir: $FORGE_MC_DIR"
    echo "========================================="
}
...
mc_server_environment() {
    ...
    sudo mkdir -p "${FORGE_MC_DIR}"
    sudo chown -R "${MC_USER}:${MC_USER}" "${FORGE_MC_DIR}"
    ...
    if [ ! -f "/etc/systemd/system/${MC_SERVICE_NAME}.service" ]; then
        ...
        if [ ! -f "${FORGE_MC_DIR}/${FORGE_MC_INSTALLER_JAR}" ]; then
            echo "⬇️ Downloading Forge Minecraft..."
            sudo -u "${MC_USER}" wget "${FORGE_MC_INSTALLER_URL}" -O "${FORGE_MC_DIR}/${FORGE_MC_INSTALLER_JAR}"
            sleep 1
        fi

        # Install server
        echo "🔨 Installing Forge Server (this may take a moment)..."
        # Forge installation often requires more RAM, ensure t3.medium or larger
        sudo -u "${MC_USER}" java ${JVM_ARGS} -jar "${FORGE_MC_DIR}/${FORGE_MC_INSTALLER_JAR}" nogui --installServer
        sleep 2
        
        echo "📜 Accepting EULA..."
        sudo -u "${MC_USER}" bash -c "echo 'eula=true' > ${FORGE_MC_DIR}/eula.txt"
        sleep 1
        
        # Create start and stop scripts
        echo "📝 Creating start/stop scripts..."
        sudo -u "${MC_USER}" bash -c "echo '${JVM_ARGS}' > ${FORGE_MC_DIR}/user_jvm_args.txt"
...
sudo -u "${MC_USER}" pkill -TERM -f "${FORGE_MC_DIR}"

# Wait for the server to actually close (loop)
while pgrep -f "${FORGE_MC_DIR}" > /dev/null; do
...
[Service]
User=${MC_USER}
WorkingDirectory=${FORGE_MC_DIR}
ExecStart=${FORGE_MC_DIR}/start
StandardInput=null
[Install]
...
EOF"
        ...
        echo "|---------------|-----------------------|"
        echo "| 📦 Version    | Forge ${FORGE_MC_VERSION}  |"
        echo "| 👤 User       | ${MC_USER}             |"
        echo "| 📂 Directory  | ${FORGE_MC_DIR}   |"
