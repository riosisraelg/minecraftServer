...
class StatusCache {
    ...
    start() {
        ...
        console.log(`[StatusCache] Started polling for ${this.instanceId} every ${this.pollIntervalMs}ms`);
    }
...
// Singleton instance (created on first require with config)
let instance = null;

/**
 * Initialize the status cache singleton.
 * @param {string} instanceId - EC2 instance ID
 * @param {number} pollIntervalMs - Polling interval in milliseconds
 * @returns {StatusCache}
 */
function initStatusCache(instanceId, pollIntervalMs = 10000) {
    if (!instance) {
        instance = new StatusCache(instanceId, pollIntervalMs);
        instance.start();
    }
    return instance;
}
...
/**
 * Get the status cache singleton.
 * @returns {StatusCache|null}
 */
